...
class EventHelper
{
    ...
    public static function registerOnOpen(EventRegister $register){
        $register->set($register::onOpen,function(\swoole_server $server,\swoole_http_request $frame){
            // p($frame);
            // p($_GET);
            echo 'onopen--'.$frame->fd."\n";
        });
    }
    public static function registerOnClose(EventRegister $register){
        $register->set($register::onClose,function(\swoole_server $server,int $fd, int $reactorId){
            // file_put_contents('aaa.php', $frame->fd, FILE_APPEND);
            // Di::getInstance()->get('REDIS')->handler()->hset('fd_',$frame->fd,'');
            echo 'onClose--'.$fd."\n";
            // p($server);
            \App\Socket\Logic\Room::recyclingFd($fd); //remove Redis
        });
    }
}